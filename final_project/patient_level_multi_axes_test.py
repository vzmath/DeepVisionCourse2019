...
from dataset_wrapper import TestMIPDatasetWrapper
from utils import patient_level_label_multi_axes, load_trained_model
...
pos_sheet = 'occlusion'
mips_axes = ('y', 'z')
models = ('model_coronal_mips.pt', 'model_axial_mips.pt')
thickness = 20
...
for filename, label in tqdm(ref_dict.items()):
    ...
    multi_labels = []
    for mips_axis, model in zip(mips_axes, models):
        # generate MIPs dataset along the current mip_axis
        ...
        # load corresponding axis-wise model
        trained_model = load_trained_model(model, NUM_CLASSES, device)
        mips_preds = dict()
        with torch.no_grad():
        ...
        multi_labels.append(pred_labels)

    patient_label = patient_level_label_multi_axes(multi_labels, thresholds)
    print(patient.get_patient_id(), label, patient_label)
    if label == patient_label[0]:
        correct_preds += 1
        if label == 0:
            correct_neg_preds += 1
        else:
