...
from dataset_wrapper import MIPDatasetWrapper
...
NUM_WORKERS = 20
MODEL = 'resnet152'
TRAINED_MODEL = 'model_axial_mips_with_neighbors.pt'
NUM_CLASSES = 2
LEARNING_RATE = 1e-3
MOMENTUM = 0.9
WEIGHT_DECAY = 1e-5
...
def train_model(device, model, dataloaders, criterion, optimizer, scheduler, num_epochs=25, is_inception=False):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for phase in ['train', 'validation']:
            ...
            for inputs, labels in dataloaders[phase]:    # since I am using my own dataset class, _ is signature
                #inputs = inputs.float()
...
def main():
    ...
    neg_mips_dataset = MIPDataset(neg_cases).get_neighbor_mips_dataset((excel_path, control_sheet), THICKNESS, False, mips_axis, NUM_NEIGHBORS)
    ...
    pos_mips_dataset = MIPDataset(pos_cases).get_neighbor_mips_dataset((excel_path, occl_sheet), THICKNESS, True, mips_axis, NUM_NEIGHBORS)
    ...
    neighbor_mips_dataset = {mip_info : (mip, mip_label) for mip_info, (mip, mip_label) in mips_dataset.items() if mip.shape[0] == 3}
    ...
    # generate datasets of neighboring mips using the original mips dataset
    mips_info = [mip_info for mip_info in mips_dataset.keys()]
    mips = [mip for mip, _ in mips_dataset.values()]
    mip_labels = [mip_label for _, mip_label in mips_dataset.values()]
    arr_len = len(mips_info)        # could be also len(mips) or len(mip_labels)
    print('Start generating neighbor MIPs dataset...')
    neighbor_mips_dataset = {mips_info[idx] : ( np.array([mips[idx-1], mips[idx], mips[idx+1]]), mip_labels[idx] )      # representing left_mip, current_mip, right_mip
                             for idx in range(1, arr_len-1)
                             if mips[idx-1].shape == mips[idx].shape and mips[idx].shape == mips[idx+1].shape}   # the filter checks if the three MIPs come from the same patient
    print('Complete generating neighbor MIPs dataset...')
    print()
    '''
    ...
    neg_mips_count = 0
    pos_mips_count = 0
    for _, label in neighbor_mips_dataset.values():
        if label == 0:
            neg_mips_count += 1
        elif label == 1:
            pos_mips_count += 1
    ...
    print('There are {} MIPs in the dataset, in which {} are negative MIPs and {} are positive MIPs.'
          .format(len(neighbor_mips_dataset), neg_mips_count, pos_mips_count))

    # split the mips dataset into train and validation sets, set up datasets and corresponding dataloaders
    train_mips_dict, val_mips_dict = train_val_split(neighbor_mips_dataset, VAL_RATIO)
    datasets = {'train' :      MIPDatasetWrapper(train_mips_dict, data_transforms['train'], neighbor_mips=True),
                'validation' : MIPDatasetWrapper(val_mips_dict, data_transforms['validation'], neighbor_mips=True)}
    ...
    print('There are {} total MIPs with {} neighbors in the training set, in which {} are negative MIPs and are {} positive MIPs'
          .format(len(datasets['train']), NUM_NEIGHBORS, train_zero_count, train_one_count))
    print('There are {} total MIPs with {} neighbors in the validation set, in which {} are negative MIPs and are {} positive MIPs'
          .format(len(datasets['validation']), NUM_NEIGHBORS, val_zero_count, val_one_count))
    print()
